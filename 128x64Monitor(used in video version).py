...
def apply_filters(image, threshold_filter=0.5):
    ...
    filtered_image = adaptive_threshold(image, threshold_strength=0.6)  # Adaptyvus slenkstis
    filtered_image = enhance_contrast(filtered_image, contrast_factor=1.5)  # Kontrasto didinimas
    filtered_image = edge_detection(filtered_image, edge_strength=0.3)  # Kontūrų aptikimas
...
def blend_images(image1, image2, alpha=0.5):
    ...
    # Užtikrinkite, kad abu paveikslėliai būtų to paties dydžio
    image2 = image2.resize(image1.size)
    
    # Konvertuoti paveikslėlius į grayscale (L) arba RGB masyvus, jei reikia
    image1_np = np.array(image1.convert("L"))
    image2_np = np.array(image2.convert("L"))
    
    # Sujungti paveikslėlius pagal nurodytą alpha koeficientą
    combined_np = (alpha * image1_np + (1 - alpha) * image2_np).astype(np.uint8)
    
    # Konvertuoti atgal į paveikslėlį
    combined_image = Image.fromarray(combined_np)
...
def calculate_dynamic_gamma(image_array, min_gamma=0.5, max_gamma=2.5):
    # Išmatuojame bendrą šviesumą (pilkos spalvos vidurkį)
    mean_brightness = np.mean(image_array)
    
    # Normalizuojame šviesumą tarp 0 ir 1
    normalized_brightness = mean_brightness / 255.0

    # Kuo paveikslėlis tamsesnis, tuo didesnė gama
    # Dinaminė gama apskaičiuojama tarp min_gamma ir max_gamma
    dynamic_gamma = min_gamma + normalized_brightness * (max_gamma - min_gamma)
...
def hybrid_dither(image, min_gamma=0.5, max_gamma=2.5, window_size=8, matrix_size=2):
    image_array = np.array(image.convert('L'))
    gamma = calculate_dynamic_gamma(image_array, min_gamma, max_gamma)
    ...
    # Pereiname per paveikslėlį langeliais
    for y in range(0, height, window_size):
        for x in range(0, width, window_size):
            # Pasirenkame langelį
            window = normalized_image[y:y+window_size, x:x+window_size]

            # Skaičiuojame vietinį kontrastą šiame langelyje
            min_intensity = np.min(window)
            max_intensity = np.max(window)
            local_contrast = max_intensity - min_intensity

            # Gauname atitinkamą slenkstinę matricą
            threshold_map = threshold_maps.get(window_size, threshold_maps[12])
            map_size = threshold_map.shape[0]

            # Pereiname per langelio pikselius
            for i in range(window.shape[0]):
                for j in range(window.shape[1]):
                    if y + i < height and x + j < width:
                        pixel_value = image_array[y + i, x + j]
                        threshold = threshold_map[i % map_size, j % map_size]  # Naudoti modulių operaciją
                        dithered_pixel = 255 if pixel_value > threshold else 0
                        dithered_image[y + i, x + j] = dithered_pixel

    return Image.fromarray(dithered_image)

def convert_to_monochrome(image):
    # Taikyti pasirinktą dithering metodą
    monochrome_image = hybrid_dither(image, min_gamma= 0.5, max_gamma=2.0, window_size=2, matrix_size=2)
...
def start_processing():
    ...
    try:
        while True:
            ...
            filtered_image = apply_filters(small_image, threshold_filter=0.65)
            
            # Konvertuoti į monochrominį su ditherinimu (kraštinių išryškinimas)
            monochrome_image = hybrid_dither(filtered_image, min_gamma= 1.8, max_gamma=4.0, window_size=32, matrix_size=12)
